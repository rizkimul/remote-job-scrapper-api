...
async def test_run_raises_if_source_not_found():
    ...
    with (
        patch.object(service._source_repo, "get_by_name", new=AsyncMock(return_value=None)),
        pytest.raises(AppError, match="not found"),
    ):
...
async def test_run_returns_success_result():
    ...
    with (
        patch.object(
            service._source_repo, "get_by_name", new=AsyncMock(return_value=_make_source())
        ),
        ...
        patch(
            "app.services.pipeline_service.deduplicate_items",
...
async def test_run_commits_on_success():
    ...
    with (
        ...
        patch(
            "app.services.pipeline_service._SCRAPER_REGISTRY",
            _make_scraper_registry([_make_raw_item()]),
        ),
        patch("app.services.pipeline_service.normalize_items", return_value=normalized),
        patch(
            "app.services.pipeline_service.deduplicate_items",
